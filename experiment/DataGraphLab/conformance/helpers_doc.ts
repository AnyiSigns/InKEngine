...
const CORE: readonly HelperRow[] = [
  ...
  { name: 'hasShortcut', signature: 'hasShortcut(task, graph): boolean', file: 'gen/producibility.ts', status: '已落地', note: 'follow 极小性守卫（R2-P0-3 删任意步版）：单步替换 + 删任意 ≥1 骨架步（真子序列 ≤31 个）回放穿验收且严格短于金计划 → 非最小，换 witness。**R5 轨迹约束后按定义恒 false**（捷径自带不同 trace → 验收拒），保留作安全网，不再滤任务' },
  { name: 'UNPRODUCIBLE_HELDOUT', signature: 'ReadonlySet<string>', file: 'gen/producibility.ts', status: '已落地', note: 'heldout 不可产注册表：goal 两族键由 goalEligible 派生（薄层视图，无第二套判定）；**R5 轨迹约束后 follow 键清空**（结构性非最小不再是缺陷，全骨架重新可产；原 80×2 键移除），覆盖构造按可产域声明、键显式上报' },
  { name: 'instanceFollow / instanceGoal', signature: 'instanceFollow(...); instanceGoal(...)', file: 'gen/generator.ts', status: '已落地', note: 'public spec + hidden gold；回放穿 accept 才返回；follow 族 spec 恒带公开 trace（R5：含收尾终算子的渲染序列，验收 = 值 ∧ hist==trace）' },
  ...
  { name: 'makeSplit', signature: 'makeSplit(split, perFamily, seed?, maxPerSkeleton?): Task[]', file: 'gen/generator.ts', status: '已落地', note: '配额制；goal 域族只走适格池（R2-P0-2）；R5 后 follow 无可产域限制（hasShortcut 恒 false、followUnproducible 退化为回放可穿），配额不足抛错不静默' },
  { name: 'makeCoverageSplitInfo', signature: 'makeCoverageSplitInfo(split, seed?, skeletons?): {tasks; unproducible; unproducibleCount; ineligible; ineligibleCount}', file: 'gen/generator.ts', status: '已落地', note: 'follow 可产域 + goal 适格池每骨架每 (style,family) 恰 1 条；goal 不适格与 follow 非最小骨架均以键显式上报（不静默），ineligible 清单仅 goal 侧；可产域内产不出抛错；骨架池可注入' },
  ...
  { name: 'safeActionSet', signature: 'safeActionSet(graph, task, start, budget): { safe: string[]; truncated: boolean; expanded: number }', file: 'data/conflict_bfs.ts', status: '已落地', note: 'Phase 2 标签软化内核（§6/C.5）：一次有界 BFS 收集「首步动作 → 通向验收」全集；start 已验收则 EXIT 安全；截断只回传已证动作（保守下界）；gold 由 oracle 按构造保证强制并入' },
  ...
  { name: 'featurizeObs', signature: 'featurizeObs(instruction, obs: ObsView, featureSet?): Float32Array', file: 'controller/features.ts', status: '已落地', note: 'lang 主臂 obs=867 维（R6 词法顺序槽 +120、R7 进度对齐槽 +15）；struct/hash_only 为诊断/消融 arch；白名单只读 instruction/state，`struct` 的 goal 段由调用方经 featurizeGoalStruct 拼接' },
  { name: 'featurizeAction', signature: 'featurizeAction(graph, nid): Float32Array', file: 'controller/features.ts', status: '已落地', note: '契约派生+哈希算子桶，ACT_DIM=83；新增算子不改宽；"any" 不置位、exit/decoy kind 独占' },
  { name: 'OBS_DIM / ACT_DIM', signature: 'OBS_DIM: Readonly<Record<FeatureSet, number>>; ACT_DIM=83', file: 'controller/features.ts', status: '已落地', note: 'lang=867 / struct=875 / hash_only=671（R6 顺序槽 + R7 进度槽后）；与 Python 侧 arch 串互钉' },
  { name: 'stateStats', signature: 'stateStats(v: unknown): readonly [number, number, number]', file: 'controller/features.ts', status: '已落地', note: '值字段统计三特征（Int 数值归一/Str 长度与字符桶）；类型 one-hot 由 featurizeState 负责' },
  { name: 'featurizeGoalStruct', signature: 'featurizeGoalStruct(spec): Float32Array', file: 'controller/features_struct.ts', status: '已落地', note: `GOAL_STRUCT_DIM=8；只进 struct 诊断 arch（G0.4 唯一例外，audit_features 执法）` },
  { name: 'Policy', signature: 'class Policy; static random(seed, featureSet?, head?); act(instr, obs, cand, graph, greedy?, rng?); save(path, trainMeta?); static load(path, expect?)', file: 'controller/policy.ts', status: '已落地', note: 'pointer 打分前向（hiddenOf/zOf/probsOf/scoresFromObs）；反向只在 train.py（F.1）；load 走 arch fail-fast 通道' },
  ...
  { name: 'beyondOracleRate', signature: 'beyondOracleRate(tasks, graph, opts?): BeyondOracleReport', file: 'eval/beyond_oracle.ts', status: '已落地', note: 'G2.2 超 oracle 率（仅 follow）：逐任务 planBfs 限深 goldLen−1 早停，非 null 记命中；超预算按保守未命名单列 overBudget，不入 hits。**R5 后真实 follow 任务按定义归零**（验收要求 hist==trace，更短解不存在），手工无 trace 任务仍走完整 BFS' },
  ...
  { name: 'PlannerArm', signature: 'class PlannerArm { solve(task, graph): RolloutResult }', file: 'eval/arms.ts', status: '已落地', note: '仅 goal：planBfs 公开规划上界（G1.3 必含臂）；无解/超预算记 accepted=false；follow N/A；产出永不回灌训练（E.13）' },
  { name: 'ContractRouteArm', signature: 'class ContractRouteArm { solve(task, graph): RolloutResult }', file: 'eval/contract_route.ts', status: '已落地', note: '两 style 廉价机制臂：公开契约反向链贪心，零学习零泄漏；对照列不进主指标；经 eval/arms.ts re-export' },
  { name: 'evaluateArm / defaultArms', signature: 'evaluateArm(arm, tasks, graph, style): ArmReport; defaultArms(randomSeed?, trained?): readonly EvalArm[]', file: 'eval/arms.ts', status: '已落地', note: '按 style 分报的统计口径（策略臂包 passAt1、replay 臂自数 accepted 共用 ci95）；N/A 不跑 solve，与 0 分严格区分' },
  { name: 'featurizeRecords', signature: 'featurizeRecords(records, featureSet?): BinRow[]', file: 'data/records.ts', status: '已落地', note: 'store 原始 obs → (obs, cand_mask, target_idx) 派生行；含 CLI main；动作表 actionFeatureTable()' },
  { name: 'writeRecordsBin / readRecordsBin', signature: 'writeRecordsBin(path, rows, obsDim, actFeats): void; readRecordsBin(path): BinFile', file: 'data/records_bin.ts', status: '已落地', note: 'records.bin 跨语言契约（magic+版本+act 表）；train.py 只读，header obsDim 与 arch 互钉' },
  { name: 'bc_train', signature: 'bc_train(D, val_D, act_table, epochs=30, patience=4, min_epochs=5, min_delta=1e-4, seed=0, batch=512, lr_schedule="cosine", head="progress", save_last_k=5)', file: 'controller/train.py', status: '已落地', note: '纯拟合、不触环境/rollout（F.1 语言分工）；val CE 早停恢复 best；落最近 K epoch 快照供 TS 按 held-out/val pass@1 选点（C.7）；train.py CLI main(argv?)：--train/--val/--out/--loss' },
  { name: 'val_ce', signature: 'val_ce(params, rows, act_table): float', file: 'controller/train.py', status: '已落地', note: '早停指标（连续 CE，非 route_acc）；batches/snapshot 非公开符号（bc_train 内联）' },
  ...
  { name: 'check_numeric_gradient', signature: 'check_numeric_gradient(seed=7, delta=1e-5, verbose=True)', file: 'controller/train_nn.py', status: '已落地', note: '变长 mask backward 数值梯度校验：‖∇num−∇ana‖/‖∇num‖ < 1e-5（D 表断言）；经 train.py --check-grad 暴露' },
  { name: 'memory_selftest', signature: 'memory_selftest(seed=0, batch=32, epochs_cap=4000, lr=3e-3, verbose=True)', file: 'controller/train_nn.py', status: '已落地', note: '记忆 32 例 → train acc ≥ 0.99 的拟合能力自检；经 train.py --selftest 暴露' },
...
const PENDING: readonly HelperRow[] = [
  ...
  { name: 'accept / acceptor_view', signature: 'accept(task, st): boolean; acceptorView(task)', file: 'verify/acceptor.ts', status: '已落地', note: '通道收口；两生产者族 verdict 判定 = verdictPass(answer)（旧 verdict 复用因指纹漂移必拒，R2-P0-1）；**R5 轨迹约束**：follow 族（spec.trace 存在）另要求 hist==trace 精确匹配，终值对但轨迹不符必拒；goal 族无轨迹约束（多解合法）' },
  ...
  { name: 'WRONG_ARTIFACTS', signature: 'readonly AdversarialCase[]', file: 'verify/adversarial.ts', status: '已落地', note: '空值/语义错/复述原题/硬编码常量+旧 verdict 复用新口径（先 check 后改值再 submit、跨任务搬运指纹、裸 "pass" 旗标——answer 达标也必拒）；**R5 增轨迹不符类**（终值对+指纹对但 hist≠spec.trace 的恒等绕路/等价重排必拒）' },
  ...
  { name: 'plan_bfs', signature: 'planBfs(task, graph, opts?): string[] | null', file: 'teacher/search.ts', status: '已落地', note: 'BFS 最短解；去重键复用 data/conflict_bfs.ts 的 stateDigest（C.4 唯一口径）；仅可解性 QA/上界诊断，不进训练集。**R5**：follow 族（spec.trace 存在）沿 trace 前缀剪枝，解唯一 = trace（等长，G2.2 归零），maxDepth 语义不变；goal 族走完整 BFS' },
