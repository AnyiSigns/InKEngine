...
const CORE: readonly HelperRow[] = [
  ...
  { name: 'dedupeBySignature', signature: 'dedupeBySignature(skels): Skel[]', file: 'gen/skeletons.ts', status: '已落地', note: '同签名留最短、同长取字典序首' },
  { name: 'SKELETONS', signature: 'readonly Skel[]', file: 'gen/skeletons.ts', status: '已落地', note: '模块级一次性计算，去冗余后全量骨架池' },
  ...
  { name: 'goalProbeHit', signature: 'goalProbeHit(root, plan, goal): boolean', file: 'gen/generator.ts', status: '已落地', note: 'Int 全域精确剪枝；Str 只提示不剪枝' },
  { name: 'INT_GOAL_POOL / STR_GOAL_POOL', signature: 'readonly Goal[]', file: 'gen/producibility.ts', status: '已落地', note: 'sampleGoal 采样空间全集（30/6），可产域判定用' },
  ...
  { name: 'UNPRODUCIBLE_HELDOUT', signature: 'ReadonlySet<string>', file: 'gen/producibility.ts', status: '已落地', note: 'heldout goal 族已知不可产域，模块加载时确定性判定（Int 全域精确、Str 有界保守）；follow 零键' },
  ...
  { name: 'makeTask', signature: 'makeTask(seed, style?, family?, split?, skeleton?): Task | null', file: 'gen/generator.ts', status: '已落地', note: '同 seed 完全确定；可钉骨架/族/切分' },
  { name: 'makeSplit', signature: 'makeSplit(split, perFamily, seed?, maxPerSkeleton?): Task[]', file: 'gen/generator.ts', status: '已落地', note: '配额制；配额不足抛错不静默' },
  { name: 'makeCoverageSplitInfo', signature: 'makeCoverageSplitInfo(split, seed?, skeletons?): {tasks; unproducible; unproducibleCount}', file: 'gen/generator.ts', status: '已落地', note: 'follow 全域 + goal 可产域每骨架每 (style,family) 恰 1 条；注册表成员记为 known-unproducible 带出计数；可产域产不出/注册表含 follow 键均抛错；骨架池可注入' },
  ...
  { name: 'append / load', signature: 'append(records, opts?): AppendResult; load(split, opts?): StoreRecord[]', file: 'data/store.ts', status: '已落地', note: '内容寻址 canonical-JSONL 分片按 (world_version, split, family)；对盘上既有步级去重；坏行 fail-fast' },
  ...
  { name: 'safeActionConflictRate', signature: 'safeActionConflictRate(records, opts?): ConflictRateReport', file: 'data/provenance.ts', status: '已落地', note: '固定 seed 抽样 ≤200 on-path 状态，金标外合法且通向验收（bounded BFS）占比；C.8 诊断项不进门禁' },
...
const PENDING: readonly HelperRow[] = [
  ...
  { name: 'accept / acceptor_view', signature: 'accept(task, st): boolean; acceptorView(task)', file: 'verify/acceptor.ts', status: '已落地', note: '通道收口；只看 public+产物' },
  ...
  { name: 'WRONG_ARTIFACTS', signature: 'readonly AdversarialCase[]', file: 'verify/adversarial.ts', status: '已落地', note: '空值/语义错/旧 verdict 复用/复述原题/硬编码常量' },
