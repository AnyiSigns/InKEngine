...
const CORE: readonly HelperRow[] = [
  ...
  { name: 'featurizeObs', signature: 'featurizeObs(instruction, obs: ObsView, featureSet?): Float32Array', file: 'controller/features.ts', status: '已落地', note: 'lang 主臂 obs=732 维；struct/hash_only 为诊断/消融 arch；白名单只读 instruction/state，`struct` 的 goal 段由调用方经 featurizeGoalStruct 拼接' },
  ...
  { name: 'OBS_DIM / ACT_DIM', signature: 'OBS_DIM: Readonly<Record<FeatureSet, number>>; ACT_DIM=83', file: 'controller/features.ts', status: '已落地', note: 'lang=732 / struct=740 / hash_only=671；与 Python 侧 arch 串互钉' },
  ...
  { name: 'HeuristicArm', signature: 'class HeuristicArm { solve(task, graph): RolloutResult }', file: 'eval/arms.ts', status: '已落地', note: '仅 follow：parseRecipe(Lexicon 首现) 线性重放；goal 抛 N/A（记 N/A 非 0）；零泄漏不触 plan_hidden/expected' },
