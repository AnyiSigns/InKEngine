...
const CORE: readonly HelperRow[] = [
  ...
  { name: 'hasOneStepSolution', signature: 'hasOneStepSolution(task, graph): boolean', file: 'gen/generator.ts', status: '已落地', note: '关死单步 echo/submit 捷径；O(|candidates|) apply+accept' },
  ...
  { name: 'makeCoverageSplit', signature: 'makeCoverageSplit(split, seed?): Task[]', file: 'gen/generator.ts', status: '已落地', note: '每骨架每 (style,family) 恰 1 条；产不出即报错' },
  { name: 'GRAPH', signature: 'Graph', file: 'runner/graph.ts', status: '已落地', note: 'OPS/NODES_BASE 组装，含 entry/exit 结构节点' },
