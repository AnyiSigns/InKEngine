...
| 符号 | 签名 | 文件 | 状态 | 语义/边界 |
|---|---|---|---|---|
| `sample_value` | `sampleValue(rng, root): number | string` | `world/operators.ts` | 待 Phase 0 | Int 全域 -50..50；Str 长度 1..8 的 a-h |
| `init_state` | `initState(x, spec?): State` | `world/operators.ts` | 待 Phase 0 | {x, answer:null, verdict:null, hist:[], spec:{}} |
| `apply_op` | `applyOp(graph, nid, st): State | null` | `world/operators.ts` | 待 Phase 0 | 契约闸+变换+hist 追加；null=死路 |
| `run_plan` | `runPlan(plan, st): State | null` | `world/operators.ts` | 待 Phase 0 | 顺序回放，不写 expected |
