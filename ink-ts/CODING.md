...
| 机制契约三键（依赖单向/装配完整/0-IO） | 各机制层 `<mechanism>/contract.ts` 全量（经 `engine/src/dock/registry/contracts.ts` 汇入 `ALL_MECHANISM_CONTRACTS`，跨 graph/gate/loop/evolve 四机制层——builder 契约已随 S1-c、kernel 层三契约 simulation/multipath/spawn 已随 P8+S1、无宿主消费遗留件 entity_evolution/evolution/growth 已随 S6 退役清零，现 24 契约）+ runtime 装配闭包 + 同机制层与 core 残部源码（扫描集仍含 kernel 历史目录名，目录已清零） | 由 `verify:mechanisms`（engine/scripts/verify_mechanisms.ts：契约密封 DAG 无环 + runtime depends 闭包 ∪ 自足叶子覆盖全量 + 机制层禁 node 内置/第三方/IO 全局原语）在 root `npm test` 与 CI 强制；boot 装配首步同源 `seal_mechanism_registry` fail-closed |
