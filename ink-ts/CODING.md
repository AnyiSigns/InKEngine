...
| 层向依赖 DAG（layer-dag） | `engine/src/{model,loop,graph,gate,evolve,dock,adapters}`（新七层；目录不存在即跳过）：model 零依赖；四件→model 放行；四件→dock 仅 `dock/ports(.ts|/*)`、`dock/registry(.ts|/*)` 前缀（写死清单不模糊匹配）；dock→四件只允许 re-export 各机制 contract.ts；adapters 只 import dock/ports 前缀与 model；四件间允许边 loop→graph、evolve→gate；未定义层间边一律违规。机制层（loop/graph/gate/evolve）红线：禁相对 import 命中旧组装模块（path_assembler/thread_skeleton/fingerprint_cache/core/assembly）、禁组装 token（组装路径/出厂图/默认拓扑/每回合拼图） | 报告模式（check.ts 打印 WARN 不阻断；P7 转强制）；豁免清单 `layerDagWhitelist` 基线为空、**单调收缩只减不增**（实施层：gate/src/layer_dag.ts） |
