...
现状散源收敛：`seed_data/tools.json`、`seed_data/ui_spec.json`、`hosts/lib/src/bridge/index.ts` 的 `BRIDGE_METHODS`、`seed_data/mcp_market.json` → 全部并入一份 `plugins/` 源；`sync_web_command_surface.ts` 从"同步命令面"扩为"从插件源生成所有派生视图"。**例外边界**：`seed_data/event_types.json` 是引擎/渲染协议侧的事件类型真源（引擎发出、渲染器消费），不属于能力插件数据，**不进 `plugins/`**，由引擎 `event_types/` 域继续作为单一事实源；web 命令面生成物（`web_command_surface.json`）属派生视图、禁手改。
...
- per-plugin AGENTS 政策：data-only 声明插件（无独占实现面）不设 per-plugin
  ...
  doc_parse + 全部 25 个真 ui 面插件 = 12 布局叶子 + 13 设置面板），细则见
...
| 2 | 命令声明即挂载样板 | 66 点分方法名归各域文件 `*_COMMANDS` 声明元组（31 份；`rounds.todos` 独立于 todos.ts 挂 rounds 域）；工厂改名 `build<Domain>Commands`，返回 `Readonly<Record<DomainCommand, BridgeHandler>>` 对象（编译期锁键集合 = 声明，缺/多/拼错即 typecheck 失败）；`BRIDGE_METHODS` 改为各域元组 spread 派生导出（值/顺序不变，self_check fixture 零漂移）；`hosts/lib/src/bridge/index.ts` 不再含任何手写方法名 | ✅ 完成 |
| 2 | verify（声明即挂载） | `verify:bridge-mount`（`hosts/lib/scripts/verify_bridge_mount.ts`）：BRIDGE_METHODS 数组体只允许 `*_COMMANDS` spread（禁点分方法名字面量防回退手写）+ spread 常量须已 import；并入 root `test` 链尾 | ✅ 完成 |
...
| 3b1 | 命令插件源 + 生成 TS 类型 | `plugins/commands/<method>/` 66 单命令目录（spec：id/kind='command'/capability='host_tool'/data.group/data.order）；`hosts/lib/src/bridge/commands.generated.ts` 生成物（31 域命令元组 + 域命令类型，DOMAIN_TABLE 固定跨域序 = BRIDGE_METHODS spread 序） | ✅ 完成 |
...
| 3b2 | canonical 白名单派生 + 消费改指 | 布局引用组件 type 并集升序 = canonical（manifest `ui_features.components` + `hosts/lib/src/bridge/ui_canonical.generated.ts`）；host recipe 界面白名单改引用生成物（手写 13 项常量删除）；web App/backend/specShell 改 import `plugins/ui.generated.json`；`seed_data/ui_spec.json` 删除 | ✅ 完成 |
...
| 5a | cli=host 插件框架 + TUI face | cli 退为**进程实现库**（`hosts/cli/src/index.ts` runCliMain 供调度；无自有语义入口），host 装配（kind=host）消费；argv 增 `tui` 形态 → `hosts/cli/src/tui/`（types/model/fmt/text/keys/actions/views/controller/tui）非 TTY line-mode 可自测 | ✅ 完成（阶段 5 首子步） |
| 5b-1 | 四份宿主 spec + host_spec loader | `hosts/tauri·cli·web·ide.spec.json`（cli/web implemented=true；tauri/ide implemented=false 占位外部壳仓）；`hosts/lib/src/host_spec.ts`（findHostsRoot/loadHostSpec/validateHostSpec）+ `verify:host-spec` 入 root test 链 | ✅ 完成 |
...
| 6 | exec 工具信封声明化 | 新 kind='endpoint' 插件域 `plugins/endpoints/`（exec/infer/mcp 三件，spec.data.native = file + env）；派生视图第 5 产物 `hosts/lib/src/exec/native.generated.ts`（NATIVE_BINARY_DECLS + NativeBinaryKind）入 verify:plugin-manifest 逐字比对；`hosts/lib/src/exec/binary.ts` 手写 BINARY_ENV/FILE_BY_KIND 两表删除、按声明定位（binaryFileName/locateNativeBinary 对外签名不变）；`_types.NativeBinaryKind` 从生成物派生；verify_unload KIND_DIRS 增 endpoints 域；插件数 131→134 数字同步 | ✅ 完成（阶段 6 目标；host 按声明装载，失败语义消费方各自定） |
| 7a | 首真面样板：doc_parse host logic face | 用户拍板：**样板真面 + 拆 1 内置示范 / 最小 logic-face 工具 / host 装配期 loader**。doc_parse 升级为首个真实逻辑脸插件：spec 顶层 `faces.logic`（target=host，entry=./faces/logic/index.ts）+ `depends=['exec']`；package.json exports node 条件导出；执行体（DocService）自 `hosts/lib/src/doc/service.ts` 迁 `plugins/tools/doc_parse/faces/logic/index.ts`，同目录 `index.test.ts` 同住；verify_unload data-only 引脚改**真面许可**（capability=external_tool 或 `REAL_FACE_BUILTINS` 白名单 doc_parse）+ faces entry 物理同住强制（相对禁逃逸 + 文件存在）；manifest 平价不变（134） | ✅ 完成 |
| 7a | host 装配期 logic-face loader | `hosts/lib/src/plugins_fs.ts`（plugins/manifest.json 探测共享，mcp.market 改指同源）+ `hosts/lib/src/face/loader.ts`（读 manifest plugins[] faces.logic.target='host' → 动态 import entry，禁越界；插件源缺 = 空集降级、face 装载失败 = fail-closed）；createHost 按声明装载 doc_parse 注入 bridge deps.docParse（rounds/material 降级语义不变） | ✅ 完成 |
| 7a | 同住纪律 + 链同步 | CODING §2.7 改「包内测试放 test/；插件 faces/impl 测试随插件同住」+ §7 表（真面插件 faces 代码入行数/UTF-8 扫描、data-only 引脚真面许可、verify 链增 `vitest run --root plugins`）；gate config lineScanDirs 增 `plugins/tools/doc_parse/faces`；hosts/lib/src/doc/service.ts 删除（旧实现真源唯一化） | ✅ 完成 |
...
| 7b | canonical 布局叶子全量真面化 | top_bar/file_tree/session_list/message_list/agent_input/review_card/task_capsule/todo_view/mechanism_view/ledger_view/trajectory_view/evolution_feed/settings_floater 13 布局叶子逐个迁 plugins faces/ui（含 gate 锚点名）；`renderer/src/app/rendererAdapters/`（layoutAdapters + index）整体退役（ledger_view 已随 9b-1b records/ledger 移除，现叶子 12） | ✅ 完成 |
...
| 物理单目录 + 真面装载 | plugins/\<kind\>/\<id\>/faces/* 物理同住（doc_parse 首个真实 host logic face：faces/logic/index.ts + 同目录 index.test.ts）+ `hosts/lib/src/face/loader.ts` 装配期按声明装载 logic face + verify 强制 faces entry 存在/禁逃逸与真面许可引脚；真 ui 面（target=web）为渲染器装配期静态注册（pluginFaces.generated.ts import entry） | ✅ 阶段 7a 样板（内置 doc_parse 真面化并拆出示范）+ 阶段 7b 全量真 ui 面（当时 26 = 13 布局叶子 + 13 settings 面板；ledger_view 移除后现 25 = 12 叶子 + 13 面板，renderer 适配层退役）；external/multi-face 走同一 seam，待真实外部插件落地 |
...
| 分层权威文档（docs/subsystems + 层 AGENTS + per-plugin AGENTS） | `docs/subsystems/{engine,plugins,renderer,host,exec}.md` 五份（定位/目录/依赖纪律/边界/验证）+ `engine/AGENTS.md`、`renderer/AGENTS.md`、`hosts/AGENTS.md` 层契约 + per-plugin AGENTS 政策（data-only 免写；真面/样板必配，25 真 ui 面 = 12 布局叶子 + 13 设置面板 + doc_parse 全量补建） | ✅ Step 3 + 收口（2026-09-08）：改哪层先读哪层的权威文档成立 |
...
### 阶段 9a 落地 + 阶段 9b 契约冻结（真 ui 面插件私有样式 / 数据接入契约）

> 阶段 9 拆两子步：9a（插件私有样式，纯展示面增量，已落地）；9b（faces/ui
> 数据接入契约 = store/inject 声明式接入，**契约本日冻结，分片实现待续**）。
> 决策留痕：#25（9a）/ #26（9b）。
...
| 10 | **B4 权限/插件收敛（2026-09-09，permission_plugin_convergence）**：设置段合并——卸载 mcp_market/tools_panel 两设置面板插件，新建 plugins 段（manifest 派生目录 pluginsCatalog + 工具常驻必带 baseline + 界面组件启停 ui_components + 服务启停 mcp.status/enable/disable（B5 后））；旧 tools_panel 权限矩阵/自动审批 UI 退役（web @app/backend setTierOverrides/setAutoApprove、renderer securityTierOverridesSet 删除；capability.tier.set 留 B6 数据面批次）；max_tool_rounds 迁入 plugins 面板 | plugins/ui_features/plugins（spec settings_section key=plugins order=10）+ hosts/web/src/app/pluginsCatalog.ts + hostAccessVocab pluginsCatalog 座位 + settingsSections/pluginFaces 重生成 | ✅ 完成（2026-09-09）：148 插件 = ui_feature 34 + 真 ui 面 24（12 叶子 + 12 设置面板）；verify:unload/gate/host-spec/plugin-manifest + hosts-web/plugins vitest 全绿。mcp.market/mount/unmount 命令面 B5 退役（见 B5 行），capability.tier.set 保留至 B6 |
| 10b | **B5 MCP 工具型插件装载（2026-09-09）**：市场命令面退役，bridge mcp.market/mount/unmount → mcp.status/enable/disable；hosts/lib McpPluginService（mcp/plugin.ts）候选目录扫描（plugins/mcp/<id>/spec.json）+ enable（connect/import_tools/声明式注册/refresh_tool_index）+ disable（unregister_definition/remove_tool_index/断连）；启用集台账 `mcp_plugins_enabled`（capability.json）boot restore 自动拉起（fail-closed）；工具可见性 = 注册表可见 + request_tool 请求即绑（不进 baseline） | engine ToolVectorIndex.remove + Runtime.remove_tool_index；hosts/lib boot.ts/index.ts/_types/bridge mcp.ts；plugins/commands mcp.status|enable|disable（真源改名）；renderer backendAdapter + hosts/web backend/PluginsPanel 服务组迁移 | ✅ 完成（2026-09-09）：hosts/lib plugin.test（假会话 seam 启停/台账/重启拉起）+ h2_wave mcp 段 + cli/renderer/web/plugins 全绿；engine 2360 passed；gate/verify 全 PASS。kind='mcp'/market.json 派生视图保留，数据面 kind 迁移单独批次 |
| 10c | **B6 agent 插件管理 + 禁停集 + 恢复默认逃生（2026-09-09）**：plugin_command 工具族（plugins/tools/plugin.* 7 工具真源 + hosts/lib/src/plugin_command.ts 端点接线 + createHost 注册）——plugin.catalog / plugin.mcp.enable|disable|install|remove / plugin.components.set / plugin.baseline.set，走统一流水线 pose 三档决；engine UI_COMPONENTS_PROTECTED（agent_input/review_card/settings_floater/message_list）+ set_ui_components_disabled 整批拒绝 + ui_protected_components/装配过滤 + ui_components.get 回显；McpPluginService.install/remove + `mcp_plugins_extra` 额外连接台账（指定安装入口防 MCP 死能力，不动 plugins 源）；recovery.settings_reset（confirm 'settings-default'）+ capability.store.reset + runtime.reset_baseline_names | engine _constants/_runtime_ui/_runtime_specs；hosts/lib mcp/plugin.ts + capability/store.ts + bridge mcp/recovery/ui_components + plugin_command.ts + index.ts；plugins/commands mcp.install|remove/recovery.settings_reset + plugins/tools/plugin.*；renderer BackendAdapter + hosts/web backend + PluginsPanel 禁停展示 + audit_recovery「恢复设置默认」块 | ✅ 完成（2026-09-09）：158 插件 = tool 45 + command 71；engine 2363 + hosts/web 44 + plugins + cli 全绿；gate/verify 全 PASS |
