...
工具声明（tools.json）随迁为 `kind:'tool'` 插件数据：内置工具 = 统一插件包（`data.tools[]` 目录铺进工具表），外部/agent 工具按插件分发；**分发单位 = 包，控制/检索单位 = 工具表行，二者正交**（细则见 PLUGINS.md §2.1）。
...
| 3 统一插件源 | `plugins/` 真源 + `manifest.json` 生成物（tools/ui_spec/mcp_market 收敛） |
