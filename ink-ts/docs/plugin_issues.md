...
| 25 | 阶段 9a 插件私有样式（CSS Modules 能力落地 + 试点）范围与语义（2026-09-08 定案，本步提交） | **A 选型 = CSS Modules**（用户拍板）：真 ui 面 faces/ui 可随插件同住 `*.module.css`（构建期作用域哈希隔离，class 名不逃逸）；共享 token/语义类仍经既有 index.css/themeTokens/designTokens 供应，禁硬编码颜色。试点 = settings_architecture：原跨包裸引 hosts/web `@app/views/w3.css`（全局样式表，唯一消费方）退役删除，改为插件本地 `faces/ui/architecture.module.css`（只迁本插件实际消费的规则子集）；被该面板共用的 hosts/web 展示资产 EmptyState 自带 `EmptyState.module.css`（同样作用域化），不再吃全局样式表。类型 = vite/client 的 `*.module.css` 声明（hosts/web tsconfig 已含，plugin faces 经 pluginFaces.generated.ts 静态 import 纳入同一 program）；plugins vitest `css:false` 下 module.css import 由 vitest stub（Proxy 占位名），测试零感知。gate 不扫 css（无机器色值约束），私有样式纪律由 plugins/AGENTS + per-plugin AGENTS 文档层守 | plugins/AGENTS.md、`plugins/ui_features/settings_architecture/AGENTS.md` + faces/ui/architecture.module.css、`hosts/web/src/app/views/EmptyState.module.css`、本卡 |
| 26 | 阶段 9b 真 ui 面数据接入契约（store/inject 声明式接入）范围与语义（2026-09-08 冻结） | 用户多轮拍板：① **只改接入面**（推翻「连渲染模型一起改」选项）：保留 spec 布局树直渲（ui.generated.json）、DynamicComponent 挂载、bind 通道、三层白名单（组件/绑定/token）不动——只把「faces/ui 拿数据的入口」从隐式/自建收敛为声明式注入；② **契约载体 = spec 声明 + 词表**（不采用 TS 契约文件）：真 ui 面 spec 声明接入面（store 读面/数据 + inject 动作/宿主回调），名称一律从单一真源词表取，禁自由命名；生成器携带声明入派生视图，verify 校验声明命中词表 + 形态合法；③ **注入实现位 = 壳装配层切片**（hosts/web 产品壳，非 renderer）：壳把全量 product chrome + 共享 backend 按每插件声明切成只含声明面的 typed props 交给 faces/ui 默认导出，渲染器 DynamicComponent 保持通用不感知插件契约；④ **推翻 #22⑤**「面板数据源 = 插件自建共享 AppBackend 单例、不经宿主注入面」→ 13 settings 面板与 canonical 叶子同走声明式切片注入，不再各自自建后端（面板也不再是唯一自建后端孤例）；⑤ 外来插件语义 = 自带 faces/ui + `*.module.css`（9a）+ spec 声明接入，装载 seam 仍走既有 external_tool 通道。分片：9b-1 契约落真源（spec 声明 JSON 形状 + 词表单一真源 + 生成器携带 + verify 校验）→ 9b-2 壳切片注入机制 + canonical 叶子迁移 → 9b-3 面板统一注入 + 纪律文档同步（per-plugin AGENTS / PLUGINS.md / checklist / CODING §10） | component_data §九「阶段 9」节、PLUGINS.md、verify_unload.ts、spec schema、hosts/web 壳装配、26 真 ui 面 faces/ui、本卡 |
| 27 | 阶段 9b-3 设置面板统一宿主注入落地（2026-09-08 完成） | #26④ 落实：mcp_market 试点先行（9b-1b 已注入 appBackend），其余 12 面板同走声明式切片注入——**settings_general 无宿主数据需求不声明**；**AppBackend 组**（tools_panel/workspace_auth/ui_editor_host）spec 声明 `faces.ui.access store:["appBackend"]`（内部组件类型 = AppBackend）；**BackendAdapter 组**（settings_architecture/connect/backup/audit_recovery/model/knowledge/memory/insights）声明 `store:["backend"]`。faces/ui 改消费注入座位：组件增可选 `backend` prop（缺省回落 `createBackend/createAppBackend`，壳外挂载/测试不白屏），knowledge/memory/insights 的 ops 工厂（createKnowledgeOps/createMemoryOps/listAudit）改接显式 adapter（消面板内多条连接），settings_architecture 入口以共享 adapter 构建 live ArchitectureBackend。切片注入位 = settings_floater 内容区经 `sliceUiAccess` 按声明切片（与 canonical 叶子 UINodeView 同机制）；自建单例收口，无面板再模块级建后端。验证：生成器重跑 + hosts/web typecheck ✓ + 受影响面板 vitest 66 ✓ | 11 面板 spec.json faces.ui.access、tools_panel/workspace_auth/ui_editor_host/index.tsx、settings_architecture/index.tsx、connect/backup/audit_recovery/model/knowledge/memory/insights faces/ui、component_data §九「阶段 9」9b-3 行、plugins/AGENTS.md + 11 per-plugin AGENTS、本卡 |
| 28 | dev 单命令启动 + 弃 .env（2026-09-08 定） | 用户要「启动 web 自动拉起后端 serve、清理 .env」：hosts/web vite dev 内置插件 `ink-ts-auto-serve`——mode=development 且未显式设 `VITE_SERVE_URL` 时自动 spawn `bootstrap/main.ts serve --port 18731 --token ink-ts-dev-loopback`（回环固定端口 + dev token，token 仅 dev/回环），并把 URL/token 注入 process.env（VITE_* 前缀 Vite 最高优先 → import.meta.env 直达 transport，语义等价旧 .env）；serve 退出码非 0 打印「端口占用/外部 serve 提示」；已显式设 serve 环境（外部 serve 联调）不拉起不覆盖。`hosts/web/.env.local` 删除；根 scripts 增 `dev:web` = `npm run dev --workspace hosts/web`（dev 单命令起前后端）。vitest（mode=test）不 spawn 不注入，测试零感知。生产/独立 serve 形态不变（另行设 VITE_SERVE_URL 或直跑 bootstrap serve） | hosts/web/vite.config.ts、ink-ts package.json `dev:web`、删除 hosts/web/.env.local、本卡 |
