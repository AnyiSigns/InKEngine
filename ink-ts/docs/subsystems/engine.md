...
测试镜像 `engine/test/`），
现 24 项分布 loop(7)/gate(7)/evolve(9)/graph(1)（builder 契约已随 S1-c 退役、
kernel 残部三项 simulation/multipath/spawn 契约已随 P8+S1 展开段退役清零、
无宿主消费遗留件 entity_evolution/evolution/growth 契约已随 S6 删除），统一经
...
- 机制契约三键（`verify:mechanisms`，现 24 项）：依赖单向 DAG、runtime
