 ...
 * 3. 0-IO：graph/gate/loop 机制层零自持 IO——禁 node:*（除 node:async_hooks 白名单，
...
/** 0-IO 扫描目录（P3a 起 = graph + gate + loop + evolve：builder/executor 迁入 graph 后覆盖等价；
...
function run(): number {
  ...
  if (failures.length === 0) {
    console.log(
      `verify:mechanisms PASS —— 契约 ${ALL_MECHANISM_CONTRACTS.length} 项密封/闭包完整，graph/gate/loop/evolve 机制层零自持 IO`,
    );
    return 0;
  }
