...
- `eventTypeSpecs.ts` — 声明族：附件/审计（5 类）/组装候选/时间线（4 类）规格函数与 `register_*` 注册辅助。
...
- 下游（实际 import 本目录）：`src/index.ts`（公共面）、`kernel/runtime`（装配注册表）、`kernel/settle`（review/promotion 审计常量）、`kernel/path_assembler`、`kernel/multipath`、`kernel/self_proposal`、`adapters/boot`、`core/fingerprint_cache`、`core/edge_evidence`；`hosts/lib/src/bridge/path.ts` 经公共面取审计常量。
