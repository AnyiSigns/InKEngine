...
| `eventTypeSpecs.ts` | 声明族规格函数（附件/审计/时间线；组装候选族随 W7-B 退役删除）+ `register_*` 注册辅助。 |
...
- 公共面：`export * from registry.js` + `export * from eventTypeSpec.js`；`eventTypeSpecs.js` 仅具名导出事件名常量（`EVENT_AUDIT_JUNCTION`/`EVENT_AUDIT_POLICY_REVIEW`/`EVENT_AUDIT_PROMOTION`/`EVENT_TURN_STARTED`/`EVENT_EXECUTION_STARTED`）与规格函数（`attachment_event_spec`/`audit_event_specs`/`output_gate_event_specs`）；`register_*` 注册函数族不随公共面外泄（装配期内部动作，宿主经 `EventTypeRegistry` + 规格函数显式装配）。`EventTypeRegistryLike` 经 `registry.ts` re-export 间接上公共面。**W7-B 落地状态**：组装载具事件常量（`EVENT_AUDIT_ASSEMBLY`/`EVENT_ASSEMBLY_CANDIDATE`/`EVENT_AUDIT_FINGERPRINT_REPLACE`/`EVENT_ASSEMBLY_STARTED`/`EVENT_ASSEMBLY_DONE`）与 `assembly_candidate_event_spec` 已随组装链路退役删除。
...
- 事件名常量：审计族 3（junction_verdict_audit/policy_edge_review_audit/recommended_prior_promotion）+ 时间线 2（turn_started/execution_started）（assembly_audit/fingerprint_replace_audit/assembly_candidate/assembly_started/assembly_done 已随组装链路退役删除，W7-B）。
...
- 命名与用途词不一致：`output_gate_event_specs`/`register_output_gate_event_types` 产出的两个时间线规格 `meta.purpose` 均为 `'timeline'`——函数名（output_gate）与 purpose 词（timeline）两套表述并存，对应关系未见显式说明。
