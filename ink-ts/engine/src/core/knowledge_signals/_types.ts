 ...
 *
 * 五类信号标签/来源分级/触发阈值单点定义（蒸馏建链角色 = router 功能槽，
 * 见 distill_role.ts，不在本模块另设常量）；SOURCE_* 经 knowledge_set
 * 重导出（外部消费方沿用 knowledge_set.SOURCE_* 形态，本模块不另起一套
 * 来源枚举）；SOURCE_RANK 为模块级唯一可信度排序基准，确定性蒸馏与「复用
