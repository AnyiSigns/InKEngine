/**
 * 事件端口面（计划 §4.1 events 拆分）：EngineEvent 封套 + EngineEventInit
 * 构造形态 + EngineTransport 传输接口 + 内存收集器/逐条容错解析。
 *
 ...
 *
 * 协议演进策略：版本化结构（PROTOCOL_VERSION 常量，真源 model/events）+
 * payload 增量演进（加字段不破坏，step_id/round_id 语义长期稳定）；破坏性
 * 变更升版本，不兼容版本在传输入口拒绝（ProtocolVersionError）。
 *
 * 传输接口化：EngineTransport = 事件消费者（SSE/WS/队列可换实现），引擎只
 * 负责产出事件流，消费方式由宿主注入（引擎提供内存传输/收集器）。
 *
 * Python 侧的 warning 留痕属可观测性副作用，TS core 零 IO 不落；容错语义
 * （跳过返回 null/字符串化降级）原样保留。
 */

import { isRecord, type JsonRecord } from '../../model/json.js';
import { PROTOCOL_VERSION, ProtocolVersionError } from '../../model/events/events.js';

export { PROTOCOL_VERSION, ProtocolVersionError };
