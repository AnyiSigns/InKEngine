...
- 下游（实际 import 本目录）：`kernel/runtime/_runtime_boot.ts`（`ALL_MECHANISM_CONTRACTS` + `seal_mechanism_registry` boot 首步密封）；全部 31 个机制件 `contract.ts`（type import `MechanismContract`；其中 13 个另取端口常量）；`engine/scripts/verify_mechanisms.ts`（verify:mechanisms 三键）；`plugins/scripts/verify_unload.ts`（`MECHANISM_PORT_IDS`，唯一跨进引擎内部取端口词表）；测试 `test/kernel/registry/`（registry.test、contracts_registry.test）；公共面 `src/index.ts` 未导出本目录（内部面）。
