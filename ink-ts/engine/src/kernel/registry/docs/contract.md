...
| `contracts.ts` | `ALL_MECHANISM_CONTRACTS` 单一真源聚合（34 项，只 re-export 不加边） |

注：任务口径中的 registry `contract.ts` 实际不存在——机制契约 `contract.ts` 落在
各机制件目录（34 份），本目录只有契约类型 `contract_types.ts` 与聚合
`contracts.ts`。
...
- 全量契约：`ALL_MECHANISM_CONTRACTS`（34 项，id 与目录同集）。
...
- 装配：`kernel/runtime/_runtime_assemble.ts` `_assemble` 首步调
...
- 契约声明侧：34 个机制件 `contract.ts` 经 `contract_types.js` 取
  `MechanismContract` 类型；其中 15 个（audit_log/executor/growth/llm/
  evolution_writer/multipath/path_assembler/pool_governance/recovery/runtime/
...
- 全量集不变式（镜像测试强制）：契约 id 全局唯一且数量 = 34；每机制 effects
  只引用 `MECHANISM_PORT_IDS` 内端口；全量依赖图无环（executor↔path_assembler
  历史环已拆，executor 经注入 seam 消费组装上下文）。
...
- `registry.test.ts` — 样板契约（audit_log）通过校验进装配序；重复 id/未知
  ...
  被依赖者先。
- `contracts_registry.test.ts` — 全量 34 机制契约：id 全局唯一且与目录同集、
...
- `MechanismContract.inject?` 注释「boot 密封时调用一次，宿主装配期注入端口实现」：`seal_mechanism_registry` 只校验并返回契约表/拓扑序，不调用 inject；34 份 contract.ts 无一定义 inject；全仓无 `.inject(` 调用点（interrupt/executor 测试中的 `inject` 属 InterruptCoordinator，另一机制）——该声明契约无执行路径。
- `MechanismContract.contract.inputs?/outputs?` 可选字段：34 份 contract.ts 无一定义使用（仅 audit_log 注释提及「声明期不固化」）——形状声明面未见消费。
