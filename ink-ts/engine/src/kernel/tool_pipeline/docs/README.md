...
- 下游（实际 import 本目录）：`src/index.ts` 公共面（`ToolPipeline`/`ToolResult` + 8 seam 类型）；`kernel/runtime`（`_runtime_assemble` 构造引擎级流水线、`_runtime_base` 持有）、`kernel/sandbox/process_sandbox`（截断常量）、`kernel/self_tools`、`kernel/introspection`（`Executor`）、`kernel/registry/contracts`；`core/nodes`（seams/llm_decider/tool_pipeline 类型）、`core/harness`、`core/declarative_tools`（`pipeline.ts` 另构造流水线实例）、`core/execution_runtime`；`adapters/mcp`（截断常量）；测试 `test/kernel/tool_pipeline`。
