# loop/round_steps — 回合步骤序列累积器（契约文档）
...
- 契约装配：`dock/registry/contracts.ts` 收入全量机制契约清单；`loop/runtime/contract.ts` 注明 round_steps 仅 type import、不构成 runtime 装配期 value 依赖。
- 值面消费：src 内 `RoundSteps` 主类无值面消费方（公共面导出供外部，`test/loop/round_steps/` 两测试直用）；`StepRecord` 类型被 `loop/runtime/runtime.ts`（`round_steps(thread)` 返回 `StepRecord[]`）与 `_round_steps_recorder.ts` 引用——后者是 runtime 侧轻量内存记录器（`EngineTransport`，复用 StepRecord 公开形态、逐事件原样投影，与 RoundSteps 主类是两条记录路径）。
...
- gate 规则：11 文件仅相对 import 与 `model/json`，无 `node:*`/第三方/宿主词命中。
- 目录形态差异：机制契约落点口径为「各机制层 `<mechanism>/contract.ts`、实现文件同住、测试镜像 `engine/test/`」——无 impl.ts 字名（实现拆分在 round_steps*.ts 八文件）、无同目录测试（见疑点 7）。

## 测试

`test/loop/round_steps/` 镜像测试两文件：
...
- 关联（非本目录镜像）：`test/loop/runtime/runtime_round_steps_recorder.test.ts` 测 runtime 侧记录器。

## 疑点与不一致

1. `dock/index.ts:233` 注释「RoundSteps 主类仍为 executor 侧消费」与代码不符：`graph/executor/` 目录经 grep 无任何 round_steps/RoundSteps 引用；src 内 `RoundSteps` 无值面消费方（仅公共面导出与测试直用）。
...
7. 目录形态：机制契约落点口径为各机制层 `<mechanism>/contract.ts` + 实现文件同住 + 测试镜像 `engine/test/`，本目录为 contract.ts + index.ts + round_steps*.ts 九文件形式（无 `impl.ts` 字名文件）。
