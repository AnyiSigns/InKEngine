...
import { Attachment, Graph as GraphImpl, system, user } from '@ink-ts/engine';
...
export function productChatGraphRecipe(ctx: GraphRecipeContext): Graph {
  ...
  const agent = async (raw: unknown): Promise<Record<string, unknown>> => {
    ...
    const messages = [
      system('ink-ts host 助手'),
      user(input, { attachments: toAttachments(nodeCtx.state['attachments']) }),
    ];
    let reply = '';
