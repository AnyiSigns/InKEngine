...
S3/S4（2026-09-14）命令与域逻辑下沉后现量 = **157 插件**（verify:unload 实测）：
tool 29 + mcp 5 + command 64 + ui_feature 32 + endpoint 3 + graph_node 8 +
ports 5 + domain 11——命令 64 全量 faces.logic 真面化、域服务 11 域插件随
hosts/lib 域逻辑下沉（S4）、端口提供方 5（storage/llm/mcp_client/boot/exec_client）；
工具面随 2026-09-15 收敛，摘除 13 个已被附件入口/受控通道/命令面覆盖的声明
（set_volume/set_brightness/system_query/sleep/notify/window_minimize/
validate_material/review_material/parse_material/mutate_knowledge/
distill_knowledge/propose_mcp_mount/material_import），tool 42→29；
