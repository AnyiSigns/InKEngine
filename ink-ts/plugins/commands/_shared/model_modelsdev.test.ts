/**
 * models.dev 能力映射测试（S3 随迁 plugins/commands/_shared）：测的是「开源
 * 注册表 reasoning_options → reasoning_style 映射 + 缓存 + 失败回退」——
 * 出网经注入桩，默认空/失败不崩。
 */

import { describe, it, expect, vi } from 'vitest';
import { fetchModelsDevCatalog, clearModelsDevCache, lookupModelsDevCapability } from './model_modelsdev.js';
import type { CatalogFetch, ModelCapabilityMap } from './model_catalog.js';
...
describe('model_modelsdev 能力映射', () => {
  ...
  it('lookupModelsDevCapability：按完整 id 精确，再按模型尾段归一匹配', () => {
    const map: ModelCapabilityMap = { 'deepseek/deepseek-v4-flash': { reasoning: true, reasoning_style: 'boolean' } };
