 ...
 * 本文件 = hosts/lib/src/bridge/rounds.ts 原样迁入（S3 命令逻辑下沉，语义零改）。
 *
 * 关键：rounds.send/abort/resume 共享**每宿主可变闭包状态**（串行回合队列
 * queue、在途主线登记 activeMainlineRuns、会话簿记 sessions）——per-command
 * 插件各自调用会各建一份闭包，破坏「单 host 串行跑回合」与中断注入语义。
 * 因此按 deps 对象身份（= 每 host 装配唯一）WeakMap 缓存整表：三个命令插件
 * 经 buildRoundsCommands(deps) 取到同一份处理器与共享状态。
 */
...
export type RoundsHandlers = Readonly<Record<string, BridgeHandler>>;

/** 每宿主共享状态缓存：同一 deps 对象（= 同一 host 装配）取同一份处理器，
 *  保证串行队列/在途登记/会话簿记跨 rounds.send/abort/resume 共享。 */
const roundsCache = new WeakMap<HostBridgeDeps, RoundsHandlers>();

/** rounds 方法组构造（每 host 装配闭包：串行回合队列 + 事件文件传输）。 */
export function buildRoundsCommands(deps: HostBridgeDeps): RoundsHandlers {
  const cached = roundsCache.get(deps);
  if (cached !== undefined) return cached;
  const handlers = buildRoundsHandlers(deps);
  roundsCache.set(deps, handlers);
  return handlers;
}

function buildRoundsHandlers(deps: HostBridgeDeps): RoundsHandlers {
  ...
  const send: BridgeHandler = async (raw): Promise<unknown> => {
    ...
    if (active !== undefined) {
      ...
      const injectedRound = await prepare(params);
