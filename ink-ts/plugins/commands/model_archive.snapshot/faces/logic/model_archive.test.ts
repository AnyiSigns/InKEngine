/**
 * model_archive.snapshot 命令逻辑面测试（S3 随迁 plugins/commands/model_archive.
 * snapshot）：测的是「档案快照聚合 + 推理能力字段透传」——厂商模型条目若声明
 * reasoning/reasoning_efforts，应原样进入档案行；非法档位被剔除；未声明 = 字段
 * 缺失（前端回退显示默认 auto）。
 */

import { describe, it, expect } from 'vitest';
import { collectArchiveRows, default as createModelArchiveSnapshot } from './index.js';
import type { HostBridgeDeps } from '@ink-ts/host';
import type { CatalogFetch } from '../../../_shared/model_catalog.js';
...
describe('collectArchiveRows（推理能力透传）', () => {
  ...
  it('模型条目未声明推理能力、厂商未发布、目录未命中 → 保持未知（不硬编码）', async () => {
    ...
    const snapshot = createModelArchiveSnapshot(deps);
    const out = (await snapshot({}, {} as never)) as { archives: Array<Record<string, unknown>> };
