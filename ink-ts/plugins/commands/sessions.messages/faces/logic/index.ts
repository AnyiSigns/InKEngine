/**
 * sessions.messages 命令逻辑面 —— S3 命令逻辑下沉（hosts/lib/src/bridge/sessions.ts
 * 迁入，语义零改）。messages = 链记录投影（沿链主线的每轮 checkpoint state 消息
 * 增量，无第二份消息台账）；展示态优先 = host.sessions.display_messages。
 */
...
export default function createSessionsMessages(deps: HostBridgeDeps): BridgeHandler {
