/**
 * MCP ���������Թ���׮������ Python ���Ե� _FakeSession/_RejectVetting/
 * _AcceptVetting ��̬��������ʵ stdio ���������� node echo server װ�䡣
 */
...
import type { McpVettingLike } from './manager.js';

/** 本地 vetting 结果形状（门禁判定只喂 verdict/reason；不引 gate 私有类）。 */
export interface VettingOutcome {
  ok: boolean;
  verdict: string;
  reason?: string;
  quarantine_tool?: boolean;
  shadow_evidence?: unknown;
}
...
export class RejectVetting {
  ...
  async vet(manifest: { name: string }): Promise<VettingOutcome> {
...
/** 全通过 vetting（不引 gate 私有类：结构满足调用方 McpVettingLike 即插即用）。 */
/** 全通过 vetting（结构满足 McpVettingLike）：vet 直接 VERIFIED；
 *  shadow_run = 真实影子执行——执行当前探针 executor 捕获写/输出/错误、
 *  恒标记 untrusted（与下沉前 ToolVetting.shadow_run 同语义：虚拟化写通过
 *  探针自身回执承载，此处不碰真实工作区）。 */
export class AcceptVetting {
  async vet(_manifest: object): Promise<VettingOutcome> {
    return { ok: true, verdict: VettingVerdict.VERIFIED };
  }
  ...
  async shadow_run(
    ...
    try {
      ...
      return new ShadowRunResult({ ok: true, output, writes: [], untrusted: true });
    } catch (error) {
...
/** ��Ψһ��ʱĿ¼���Ǽǵ������嵥�������ļ��� cleanup_tmp_dirs ͳһ�������� */
...
/** ��С MCP stdio server��node ���ļ�����echo_text + ping + ����ע�롣 */
...
/** echo server �� stdio �������ã�env װ�ع���ע�뿪�أ��� */
