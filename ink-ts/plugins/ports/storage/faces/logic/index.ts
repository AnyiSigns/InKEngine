 ...
 * 只装载不解析；storage 实例 = 工厂具名展开面，仓库装配按 manifest「ports」
 * 段取用）。
 */
export default function createStoragePort(): Record<string, unknown> {
  return { create_storage, MemoryStorage, create_memory_storage, SqliteStorage };
}
