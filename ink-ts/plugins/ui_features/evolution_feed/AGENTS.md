...
状态页主视图（canonical 叶子）：孵化/补丁动态时间线 + 当前回合图组成清单（执行过结点/走过的边/召唤占位，取代已废弃的 DAG）+ 协作项目录（只读）。
...
- chrome 读面：backend（available:false → 禁用态空态占位）+ activeSessionId；实例快照映射经 hosts/web 产品壳 app/views/architecture（mapInstanceSnapshot），只读成分清单，不渲染 DAG。
