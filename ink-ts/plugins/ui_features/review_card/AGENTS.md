...
审批卡浮层（canonical 叶子）：events.review_card 绑定的审批决议卡，读 chrome.pendingReview 载荷并触发决议动作（onResolveReview 续跑对应线程）。
...
- chrome 读面：pendingReview / review 决议动作（onResolveReview 等），events.review_card 事件绑定为入场（ev = 卡载荷，thread 续跑）。
