...
- 接入（声明式）：faces.ui.access store:["backend"]（见 spec.json）——壳 settings_floater 经 sliceUiAccess 按声明切片注入共享 BackendAdapter，入口以其构建 live ArchitectureBackend；壳外挂载/测试缺省回落自建（dev 夹具）。
