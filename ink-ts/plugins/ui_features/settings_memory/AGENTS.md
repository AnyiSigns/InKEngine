# settings_memory（per-plugin AGENTS）
...
设置「记忆」段：记忆面/上下文展示与管理入口。

## 数据从哪进 / 挂载

- 挂载 = 设置面板：不经布局树 $ref——spec data.settings_section 经生成器 settingsSections.generated.ts（住 hosts/web/src/app/settings/）settings 派生清单引用，设置浮层读清单渲染左导航、内容 DynamicComponent name=插件 id（settings_memory）；真源注册同 pluginFaces.generated.ts。
- 数据：@app/backend 共享数据面（memory.list/invalidate）或 chrome；写记忆走引擎受控通道，本段只出动作出口。
