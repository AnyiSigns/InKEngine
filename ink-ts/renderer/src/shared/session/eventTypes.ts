...
export const EVENT_TYPE_NAMES = [
  ...
  'error',
  'end',
  // 节点追踪（组装候选与组装/指纹审计已随组装链路退役，W7-B）
...
export const EVENT_TYPE_SPECS: EventTypeSpec[] = [
  ...
  { name: 'plan_token', description: '规划流式增量 token', bindable: true },
  ...
  { name: 'node_end', description: '节点执行完成（架构实例追踪回填）', bindable: true },
  { name: 'node_fail', description: '节点执行失败（架构实例追踪回填）', bindable: true },
  ...
  { name: 'output_verdict', description: '节点产出评审裁决（VTM 验证器留痕）', bindable: true },
