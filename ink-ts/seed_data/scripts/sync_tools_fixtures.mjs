#!/usr/bin/env node
/**
 * 同步生成工具声明夹具（fixtures/tools_os.json），plugins 为唯一真源。
 *
 * ink-ts 产品级生成管线，供 host/exec/web 消费前与 plugins 漂移校验。
 * 职责边界：
 * - plugins/manifest.json = 插件源派生视图（tools = kind='tool' 工具表行
 *   聚合，由 plugins/scripts/sync_plugin_manifest.mjs 从各 spec 派生）；
 * - fixtures/tools_os.json = 执行器声明**生成物**——从 plugins manifest 派生，
 *   禁手工维护（生成产物与 plugins 成员/档位/端点漂移由 --check 模式硬校验）。
 *
 * 成员集合规则（生成器显式映射，勿静默改动）：
 * - 种子 = plugins 中 meta.domain == "os" 的 OS 域工具（插件源的 OS 能力面）；
 * - 追加 = 执行器实现的非 OS 域工具（文档/导入/自指演化，固定清单）；
 * - shell_exec = 工作区命令执行器（混合级别：白名单内命令 cwd 钉工作区
 *   挂载根；白名单外命令经引擎升级审批通过后一次性系统级放行）。
 *
 * schema 形态差异映射（plugins 嵌套 schema → 夹具扁平签名）：
 * - 参数：取 plugins 行 parameters.properties 中除固定 command 枚举外的参数；
 * - 端点：plugins endpoint（process_exec / mcp）→ process_exec / device_mcp
 *   （mcp + inkling_shell 的感知类，见 MCP_TO_DEVICE）；
 * - 档位：plugins approval（allow/review/deny）→ 夹具 permission；
...
const PLUGIN_MANIFEST = join(SEED_ROOT, '..', 'plugins', 'manifest.json');
...
const FIXTURE_NOTE =
  '执行器声明生成物：由 plugins/ 真源（manifest tools 聚合）经 ink-ts/seed_data/scripts/' +
  'sync_tools_fixtures.mjs 生成（plugins=插件源真源，fixtures=执行器声明，' +
  '禁手工维护）；成员 = plugins OS 域工具 ∪ 执行工具（shell_exec 为混合级别：白名单内' +
...
function assertSeedParamsShape(name, seedTool) {
  ...
  if (typeof props !== 'object' || props === null) {
    fail(`工具 ${name} 缺 parameters.properties（plugins 声明不完整）`);
  }
  ...
  if (unexpected.length > 0) {
    fail(`工具 ${name} 的参数未在生成映射中: ${JSON.stringify(unexpected.sort())}（新增参数须显式登记到 PARAMS_MAPPING）`);
  }
}

async function deriveFixture(pluginsManifest) {
  const tools = pluginsManifest.tools;
  if (!Array.isArray(tools)) fail('plugins manifest 缺 tools 清单');
  ...
  if (missing.length > 0) fail(`成员集合含 plugins 缺失工具: ${JSON.stringify(missing)}`);

  for (const exempt of Object.keys(FIXTURE_EXEMPTIONS)) {
    const tool = byName.get(exempt);
    if (!tool) fail(`豁免项  在 plugins 中不存在（豁免登记失真）`);
...
async function main() {
  ...
  const pluginsManifest = await loadJson(PLUGIN_MANIFEST);
  const generated = await deriveFixture(pluginsManifest);
  ...
  if (check) {
    ...
    if (current !== rendered) {
      console.error(`夹具漂移:  与 plugins 派生产物不一致（重跑 sync_tools_fixtures.mjs）`);
      process.exitCode = 1;
      return;
    }
    console.log('夹具与 plugins 派生产物一致');
    return;
  }

  await mkdir(dirname(FIXTURE), { recursive: true });
  await writeFile(FIXTURE, rendered, 'utf8');
  console.log(`已生成 ${relative(SEED_ROOT, FIXTURE)}（${generated.tools.length} 件声明，plugins 真源）`);
}
