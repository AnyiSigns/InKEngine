 ...
 *
 * 扫描范围：
 * - 定义抽取：engine/src/core、engine/src/model、engine/src/graph 与 engine/src/gate 下所有 .ts 的顶层 `export function|class|enum|const`；
...
function collect(ctx: SelfCheckContext): { defs: Def[]; counts: Map<string, number> } {
  ...
  const walk = (dir: string, base: string): void => {
    for (const file of walkFiles(dir)) {
      ...
      addTokens(text);
      if (relPath.startsWith('engine/src/core/') || relPath.startsWith('engine/src/model/') || relPath.startsWith('engine/src/graph/') || relPath.startsWith('engine/src/gate/')) {
        DEF_RE.lastIndex = 0;
        let m: RegExpExecArray | null;
...
export async function runGateSymbols(ctx: SelfCheckContext): Promise<GateResult> {
  ...
  return {
    key: 'symbols',
    label: '符号引用计数',
    command: 'engine/src/core、engine/src/model、engine/src/graph 与 engine/src/gate 顶层导出孤儿扫描',
    passed,
    seconds,
    summary,
    tail,
