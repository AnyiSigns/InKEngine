"""上下文调配器原语（D7 Context Mixer：多源融合的确定性层 + LLM 融合钩子）。

创作上下文多源注入（RAG 检索片段/书级记忆/设定场景卡/模拟支线/读者反馈），
...
class ContextSource:
    ...
    Attributes:
        type: 源类型（业务自定义：chapter/memory/character/world/...）。
        content: 源正文（空内容 = 无意义源，分配时剔除）。
