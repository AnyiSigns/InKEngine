...
可嵌入的通用 agent 机制运行时。一切演化都是补丁链数据——每次变化经
审批（L0/L1/L2 分级）、可审计（append-only）、可回退（链尾单步），
演化全程受控。

| 文档 | 内容 | 面向 |
|---|---|---|
| [concepts.md](concepts.md) | 概念体系：卡回路/时间线/发散收敛/推演/重规划/声明式化/知识孵化/调配/自指演化/实体协作者/成长管线/安全纵深等 | 心智模型与宣传 |
...
| [evidence_arbitration.md](evidence_arbitration.md) | 记忆整合仲裁：缓存短路/边证据/策略边 τ 档的裁决链（设计说明） | 机制深入 |
| [whitelist_audit.md](whitelist_audit.md) | 机制层硬编码清单审计（装配数据化 / 机制固有二分定界） | 审计 |
| [executor_split_report.md](executor_split_report.md) | executor.py 拆分决策记录（#8，历史决策文档，已归档） | 历史 |

代码在 `ink_engine/core`（engine-core，纯机制唯一 seam）；领域深度归
宿主产品层（领域规则/样例/谓词由产品自写并成对维护），机制层只带
通用种子与 boot 自举基线（`ink_engine.seeds`）；InKling 受控自进化
智能体（`inkling/`）与 stdio_host 为本包的两个宿主参考实现，读取
core API 与种子数据组装自身流程，不复写机制；`examples/` 附可独立
运行的示例。
