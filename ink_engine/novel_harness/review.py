...
from ink_engine.novel_harness.candidate_mix import split_paragraphs
...
async def run_review_convergence(
    ...
    *,
    ...
    宿主可选扩展：循环与再生成（NovelRegenerator）是否接线由宿主决定——
    发散挑选主流程未启用自动收敛（保留多版本选择空间），本原语适合
    单稿精修 / 呈交前质量门等场景按需接入。
    ...
    while True:
        ...
        if decision.converged:
            # 防御外部策略越界下标（公共原语不自防御则自定义策略可触发 IndexError）
            accepted = [
                current[i]
                for i in decision.accepted_indices
                if 0 <= i < len(current)
            ]
            return ConvergenceResult(
                candidates=accepted,
