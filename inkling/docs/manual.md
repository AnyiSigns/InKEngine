...
1. **深认知**：越用越懂领域——知识孵化（信号五类 → 蒸馏 → 三层闸门 →
   work/project/user 分层晋升）、记忆沉淀（本地 md 文件，用户可读可编辑）、
   边证据演化（路径统计按域聚合，信任档观察/常规/转正自动晋级）。
...
| 知识孵化 | 信号（pitfall/纠偏/insight/缺口/根因）→ 蒸馏 → L1 安全扫描 / L2 形态语义 / L3 目标筛查 → 分层晋升 → KNOWLEDGE 补丁落链 |
...
| 设置页四节 | 环境容器 / 应用能力（推演档、推理档、模型配置、web_search key）/ 生长治理 / 安全信任（权限矩阵、网络策略、崩溃回退、导出备份） |
