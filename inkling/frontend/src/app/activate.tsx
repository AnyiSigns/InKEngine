 ...
 * 最终接线由后继集成 agent 完成。
 *
 * SettingsSectionSpec 契约（波 2 registry 签名）：
 * { key, label, icon, order, items? | render? }
 *
 * SettingsItemSpec 契约（波 2 registry 签名）：
 * { key, label, hint?, kind, options?, read, write?, disabledReason?, validate? }
 */
...
import { McpMarket } from './views/markets/McpMarket';
import { ComponentMarket } from './views/markets/ComponentMarket';
import { ToolsPanel } from './views/tools/ToolsPanel';
import { OsView } from './views/os/OsView';
import { WorkspaceAuth } from './views/workspace/WorkspaceAuth';
import { EnvironmentContainer } from './views/workspace/EnvironmentContainer';
import { UiEditorHost } from './views/uieditor/UiEditorHost';
      ...
      items: [
        {
          key: 'ui_editor_host',
          label: '界面树编辑器',
          hint: 'inspect_ui 拉取 setLiveSpec；产物到补业链落链可回退',
          kind: 'component',
