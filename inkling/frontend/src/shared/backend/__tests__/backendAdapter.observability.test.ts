 ...
 * mock 后端契约（modelsSnapshot/metricsSnapshot/assembleStats/四 op）。
 */
...
describe('mock 后端契约', () => {
  function mockBackend(overrides: Partial<BackendAdapter> = {}): BackendAdapter {
    return {
      available: true,
      modelsSnapshot: vi.fn(async (): Promise<ModelsSnapshot> => ({ profiles: [] })),
      ...
      restoreEdgeTier: vi.fn(async () => ({ edge: 'e1', tier: 'high' })),
      ...overrides,
    } as BackendAdapter;
  }
