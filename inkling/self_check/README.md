...
crate，避免 tauri/pyo3/ort 编译地狱）。四门禁一键矩阵化报告，命令事实源
= `inkling/manifest.json` 的 `self_check` 表（脚本不重复声明命令）。

## 子命令

| 子命令 | 门禁内容 | 超时 |
| --- | --- | --- |
| `schema` | seed_data 17 文件逐文件 schema 校验 + 跨文件一致性 + manifest 身份定稿 + boot_prompt 定稿 + 引擎源码事实核对（AssemblyRecipe 字段数/引擎版本）+ 工具名长度 ≤ 24 全量 + 校验器自检夹具 | 秒级 |
