/**
 * 设置页表单（三级信息架构最深层：改一次很久不变的系统配置）。
 *
 * 区块（按 PLAN §5.2 设置页定稿）：
 * 模型 / 权限与审批 / 连接（MCP 挂载管理 + 环境管理 + 工作区授权）/
 * 数据与记忆 / 外观（主题 token 试穿再应用，白名单内）/ 关于。
 *
...
import {
  ...
  Info, KeyRound, Paintbrush, PlugZap, RotateCcw, Upload,
} from 'lucide-react';

import { applyThemeTokens, rejectedThemeTokens, THEME_TOKEN_DEFAULTS, THEME_TOKEN_WHITELIST } from '@/renderer/themeTokens';
...
export function SettingsForm({ bindValue, onApplySettings }: SettingsFormProps) {
  void bindValue;
  ...
  const [themeDraft, setThemeDraft] = useState<Record<string, string>>({ ...THEME_TOKEN_DEFAULTS });
  ...
  return (
    <div className="space-y-3 p-3">
              ...
              <div className="flex gap-1">
                {APPROVAL_LEVELS.map((level) => (
                  <button
                    ...
                    className={cn(
                      'h-6 rounded px-2 text-[10px] cursor-pointer',
      ...
      <SettingsSection icon={PlugZap} title="连接" hint="MCP 挂载管理（市场 + 手动添加）">
        <div className="space-y-2">
          ...
          {MCP_MARKET.map((entry) => {
            ...
            return (
              <div key={entry.name} className="ink-elevated flex items-center gap-2 rounded-md px-2.5 py-1.5">
      ...
      <SettingsSection icon={AppWindow} title="环境与工作区" hint="环境声明（local/web_bridge/container）+ 工作区授权 + file_ops 分级">
        <div className="flex gap-1.5">
          {ENVIRONMENTS.map((env) => (
            <span key={env} className="ink-elevated rounded px-2 py-1 text-[10px] font-mono ink-text-muted">
              {env}
            </span>
          ))}
        </div>
      ...
      <SettingsSection icon={Paintbrush} title="外观" hint="主题 token（白名单内试穿再应用）">
        <div className="space-y-2">
          {THEME_TOKEN_WHITELIST.map((token) => (
            <div key={token} className="flex items-center gap-2">
              <span className="w-28 shrink-0 font-mono text-[10px] ink-text-muted">{token}</span>
              <div className="h-5 w-5 shrink-0 rounded border" style={{ background: themeDraft[token] }} aria-hidden />
              <TextInput
              ...
              <Button
                size="xs"
                variant="ghost"
                onClick={() => updateThemeToken(token, THEME_TOKEN_DEFAULTS[token as keyof typeof THEME_TOKEN_DEFAULTS])}
              >
                出厂默认
              </Button>
            </div>
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            <span className="text-[10px] ink-text-muted">试穿即时生效（白名单内）；「应用设置」持久化</span>
          </div>
        </div>
      </SettingsSection>
      ...
      <SettingsSection icon={Info} title="关于" hint="版本 / engine_version_compat / 契约清单">
        <div className="space-y-1 text-[10px] ink-text-muted">
          ...
          {aboutOpen && (
            <div className="rounded border px-2 py-1.5 font-mono text-[9px] ink-border ink-text-faint">
      ...
      <div className="flex justify-end gap-2">
        <Button size="sm" variant="ghost" onClick={() => setThemeDraft({ ...THEME_TOKEN_DEFAULTS })}>
          <RotateCcw size={11} strokeWidth={1.6} /> 还原
        </Button>
