 ...
 *
 * 数据源：state.sourceTraces 通道（memory_recall / vetting_result / tuning_applied /
      ...
      {traces.length === 0 ? (
        ...
        <ul className="mt-2 space-y-1.5">
          {traces.map((trace) => {
            ...
            return (
              <li key={trace.id} className="ink-elevated px-3 py-2">
                ...
                {trace.detail && <div className="mt-0.5 truncate text-[10px] ink-text-muted">{trace.detail}</div>}
                {trace.knowledgeId && (
                  <div className="mt-0.5 text-[9px] font-mono ink-text-faint">知识条目：{trace.knowledgeId}</div>
                )}
              </li>
            );
          })}
