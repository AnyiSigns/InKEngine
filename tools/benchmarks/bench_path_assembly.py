...
async def run_size(
    ...
    """单规模基准：返回 (规模, 域过滤ms均值, 检索ms均值, 组装ms均值, beam扩展峰值, 评分计算峰值)。"""
...
async def main() -> None:
    print("=" * 78)
    print("规模基准：域过滤 / 检索 top-N（内存兜底）/ beam 扩展数 / 评分计算量 / 单任务组装全程")
    print(f"合并门槛：单任务组装全程 < {RUNTIME_TARGET_MS:.0f}ms（千结点量级）")
    print("=" * 78)
    rows = []
    ...
    print(f"{'规模':<7}{'域过滤ms':<11}{'检索topN ms':<13}{'组装耗时ms':<12}"
          f"{'beam扩展':<9}{'评分计算':<9}{'达标'}")
    all_pass = True
    for size, dms, rms, ams, beam, scores in rows:
        ok = ams < RUNTIME_TARGET_MS and beam > 0 and scores > 0
        all_pass = all_pass and ok
        mark = "[达标]" if ok else "[未达标]"
        print(f"{size:<7}{dms:<11.2f}{rms:<13.2f}{ams:<12.2f}{beam:<9}{scores:<9}"
              f"{mark}")
    print("-" * 78)
